...
/* f_Phi:(i0[7],i1[3x4],i2[3x3],i3[3x2])->(o0[27]) */
...
void mex_f_Phi(int resc, mxArray *resv[], int argc, const mxArray *argv[]) {
  ...
  if (--argc>=0) arg[0] = casadi_from_mex(argv[0], w, casadi_s0, w+61);
  if (--argc>=0) arg[1] = casadi_from_mex(argv[1], w+7, casadi_s1, w+61);
  if (--argc>=0) arg[2] = casadi_from_mex(argv[2], w+19, casadi_s2, w+61);
  if (--argc>=0) arg[3] = casadi_from_mex(argv[3], w+28, casadi_s3, w+61);
  --resc;
  res[0] = w+34;
  i = f_Phi(arg, res, iw, w+61, 0);
...
casadi_int main_f_Phi(casadi_int argc, char* argv[]) {
  ...
  flag = f_Phi(arg, res, iw, w+61, 0);
  if (flag) return flag;
  r = w+34;
  for (j=0; j<27; ++j) CASADI_PRINTF("%g ", *r++);
  CASADI_PRINTF("\n");
  return 0;
}
