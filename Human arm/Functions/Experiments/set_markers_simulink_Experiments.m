function set_markers_simulink_Experiments(sh_number, fo_number, ha_number)

coder.extrinsic("add_block", "add_line", "set_param", "get_param", "strcat", "get", "num2str");
...
if (ha_number > 0)
    ...
    if (ha_number > 1)
        ...
        for i = 2 : ha_number
            ...
            pdoti = add_block(strcat(path, '/pdot_1'), strcat(path, '/pdot_1'), 'MakeNameUnique', 'on');
            %pdoti_out = get_param('master_thesis_simulink/Experiments/Human arm/RightHand/Markers/M2', 'PortHandles').LConn(2);
            pdoti_in = get_param(pdoti, 'PortHandles').Inport;
            set_param(pdoti, 'Position', current_position_pdoti + [0 80 0 80]);
            current_position_pdoti = get_param(pdoti, 'Position');
