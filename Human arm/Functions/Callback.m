...
if start
    ...
    % Position Covariance 
    covariance_pos_all = reshape(message.Pose.Covariance, sqrt(length(message.Pose.Covariance)), sqrt(length(message.Pose.Covariance)));
    covariance_pos = covariance_pos_all(1:3, 1:3);
    disp(covariance_pos);

    % Velocity Covariance
    covariance_vel_all = reshape(message.Twist.Covariance, sqrt(length(message.Twist.Covariance)), sqrt(length(message.Twist.Covariance)));
    covariance_vel = covariance_vel_all(1:3, 1:3);
    disp(covariance_vel);
end
